...
public class SurfaceVideoView extends SurfaceView implements Callback {
	...
	protected void initVideoView() {
		...
		getHolder().setFormat(PixelFormat.RGB_565); // PixelFormat.RGB_565
	...
	public void setVolume(float volume) {
		// 可用状态{Idle, Initialized, Stopped, Prepared, Started, Paused,
		// PlaybackCompleted}
		if (mMediaPlayer != null
				&& (mCurrentState == STATE_PREPARED
						|| mCurrentState == STATE_PLAYING
	...
	public int getCurrentPosition() {
		int position = 0;
		// 可用状态{Idle, Initialized, Prepared, Started, Paused, Stopped,
		// PlaybackCompleted}
		if (mMediaPlayer != null) {
	...
	public void openVideo(Uri uri) {
		...
		// Idle
		// 状态：当使用new()方法创建一个MediaPlayer对象或者调用了其reset()方法时，该MediaPlayer对象处于idle状态。
		// End
		// 状态：通过release()方法可以进入End状态，只要MediaPlayer对象不再被使用，就应当尽快将其通过release()方法释放掉
		// Initialized
		// 状态：这个状态比较简单，MediaPlayer调用setDataSource()方法就进入Initialized状态，表示此时要播放的文件已经设置好了。
		// Prepared
		...
		public void onPrepared(MediaPlayer mp) {
			...
			if (mCurrentState == STATE_PREPARING) {
				...
				try {
					...
					Log.e("SurfaceVideoView", "videoWidth:" + mVideoWidth + ", videoHeight:" + mVideoHeight);
	...
	private Handler mVideoHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case HANDLER_MESSAGE_LOOP:
				if (isPlaying()) {
					seekTo(msg.arg1);
					sendMessageDelayed(mVideoHandler.obtainMessage(
							HANDLER_MESSAGE_LOOP, msg.arg1, msg.arg2), msg.arg2);
				}
				break;
			default:
				break;
	...
	public void pauseDelayed(int delayMillis) {
		...
		mVideoHandler.sendEmptyMessageDelayed(HANDLER_MESSAGE_PARSE,
				delayMillis);
	}
	...
	public void loopDelayed(int startTime, int endTime) {
		...
		if (mVideoHandler.hasMessages(HANDLER_MESSAGE_LOOP))
			mVideoHandler.removeMessages(HANDLER_MESSAGE_LOOP);
		mVideoHandler.sendMessageDelayed(mVideoHandler.obtainMessage(
				HANDLER_MESSAGE_LOOP, getCurrentPosition(), delayMillis),
				delayMillis);
	}
