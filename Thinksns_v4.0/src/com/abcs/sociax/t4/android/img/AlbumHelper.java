package com.abcs.sociax.t4.android.img;
...
public class AlbumHelper {
	...
	void buildImagesBucketList() {
		...
		while (itr.hasNext()) {
			...
			Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", "
					+ bucket.count + " ---------- ");
			List<ImageItem> list = new ArrayList<ImageItem>();
			for (int i = bucket.imageList.size() - 1; i >= 0; i--) {
	...
	public List<ImageBucket> getImagesBucketList(boolean refresh) {
		...
		Iterator<Entry<String, ImageBucket>> itr = bucketList.entrySet()
				.iterator();
		List<ImageBucket> resultList = new ArrayList<ImageBucket>();
		while (itr.hasNext()) {
			Map.Entry<String, ImageBucket> entry = (Map.Entry<String, ImageBucket>) itr
					.next();
			ImageBucket bucket = entry.getValue();
		...
		for (int i = 0; i < tmpList.size(); i++) {
			ImageBucket ib = tmpList.get(i);
			if (!ib.bucketName.equals("Camera")) {
				resultList.add(ib);
			}
		}
		return resultList;
