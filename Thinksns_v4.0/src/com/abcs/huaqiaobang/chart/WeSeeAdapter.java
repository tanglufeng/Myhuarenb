...
public class WeSeeAdapter extends BaseAdapter{
	...
	public View getView(int position, View v, ViewGroup arg2) {
		...
		if(v == null){
			...
			Log.d(TAG, "id :"+we.uid+"  self :"+MyApplication.getInstance().self.getId());
			if(type == 0){
...
//		StartActivity.imageLoader.displayImage(holder.image.getTag().toString(), holder.image);
		...
		if(we.avatar.equals("http://user.cavacn.com:3000/images/avatar-default.jpg")){
			...
			if (holder.image.getTag() == null
					|| !holder.image.getTag().equals(we.avatar))
			{
				StartActivity.imageLoader
				.displayImage(we.avatar, imageAware, Options.getListOptions()
