...
import com.abcs.huaqiaobang.ytbt.common.utils.DateUtil;
...
public class NewsManager {
    ...
    public ArrayList<News> getNoNetNews(boolean isNews) {
        ...
        results = myRealm.where(News.class).equalTo("special", nowTypeSpecial).equalTo("subject", subject)
                .findAll();
    ...
    public static String getNewsDate(long time, boolean isPictureNews) {
        ...
        if (day.equals(dayCurrent)) {
            return isPictureNews ? getDateOnlyHour(time) : "今天     " + getDateOnlyHour(time);

        } else {
            return getDateMDhhmm(time);
        }
