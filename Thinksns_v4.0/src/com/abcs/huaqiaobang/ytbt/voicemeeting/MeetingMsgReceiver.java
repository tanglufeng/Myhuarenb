...
public class MeetingMsgReceiver implements OnMeetingListener {
	...
	private OnVideoMeetingMsgReceive onVideoMeetingMsgReceive;
	...
	public static void addVideoMeetingListner(
			OnVideoMeetingMsgReceive onVideoMeetingMsgReceive) {
		getInstance().onVideoMeetingMsgReceive = onVideoMeetingMsgReceive;
	}

	public static void removeVideoMeetingListner() {
		getInstance().onVideoMeetingMsgReceive = null;
	}
	...
	public static void removeVoiceMeetingListener(
		...
		if (remove != null) {
			...
			Log.i(TAG, "remove observer success , observer " + receiver);
		}
	}
	...
	public void onReceiveVideoMeetingMsg(ECVideoMeetingMsg msg) {
		...
		if(getInstance().onVideoMeetingMsgReceive!=null){
			getInstance().onVideoMeetingMsgReceive.onReceiveVideoMeetingMsg(msg);
		}
	}
	...
	public static void putInterPhone(String meetingNo) {
		...
		inviteMsg.setFrom(MyApplication.getInstance().getName());
		inviteMsg.setReceiver(MyApplication.getInstance().getName());
		if (mInterPhones == null) {
			mInterPhones = new ArrayList<ECInterPhoneMeetingMsg>();
		}
		mInterPhones.add(inviteMsg);
	...
	public interface OnVideoMeetingMsgReceive {
		void onReceiveVideoMeetingMsg(ECVideoMeetingMsg msg);
		void onVideoRatioChanged(VideoRatio video);

	}

	@Override
	public void onVideoRatioChanged(VideoRatio video) {
		getInstance().onVideoMeetingMsgReceive.onVideoRatioChanged(video);
//		if (video != null) {
//			Bundle b = new Bundle();
